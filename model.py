...
# CNN Encoder - ViT bottle neck - CNN Decoder : TransUnet
...
class Unet(nn.Module):
    def __init__(self):
        ...
        def CBR2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1, bias=True):
            layers = []
        ...
        self.enc1_1 = CBR2d(in_ch=3, out_ch=64)
        self.enc1_2 = CBR2d(in_ch=64, out_ch=64)

        self.pool1 = nn.MaxPool2d(kernel_size=2)

        self.enc2_1 = CBR2d(in_ch=64, out_ch=128)
        self.enc2_2 = CBR2d(in_ch=128, out_ch=128)

        self.pool2 = nn.MaxPool2d(kernel_size=2)

        self.enc3_1 = CBR2d(in_ch=128, out_ch=256)
        self.enc3_2 = CBR2d(in_ch=256, out_ch=256)

        self.pool3 = nn.MaxPool2d(kernel_size=2)

        self.enc4_1 = CBR2d(in_ch=256, out_ch=512)
        self.enc4_2 = CBR2d(in_ch=512, out_ch=512)

        self.pool4 = nn.MaxPool2d(kernel_size=2)

        self.enc5_1 = CBR2d(in_ch=512, out_ch=1024)

        # Expansive path
        self.dec5_1 = CBR2d(in_ch=1024, out_ch=512)

        self.unpool4 = nn.ConvTranspose2d(in_channels=512, out_channels=512,
                                          kernel_size=2, stride=2, padding=0, bias=True)

        self.dec4_2 = CBR2d(in_ch=2 * 512, out_ch=512)
        self.dec4_1 = CBR2d(in_ch=512, out_ch=256)

        self.unpool3 = nn.ConvTranspose2d(in_channels=256, out_channels=256,
                                          kernel_size=2, stride=2, padding=0, bias=True)
        self.dec3_2 = CBR2d(in_ch=2 * 256, out_ch=256)
        self.dec3_1 = CBR2d(in_ch=256, out_ch=128)

        self.unpool2 = nn.ConvTranspose2d(in_channels=128, out_channels=128,
                                          kernel_size=2, stride=2, padding=0, bias=True)

        self.dec2_2 = CBR2d(in_ch=2 * 128, out_ch=128)
        self.dec2_1 = CBR2d(in_ch=128, out_ch=64)

        self.unpool1 = nn.ConvTranspose2d(in_channels=64, out_channels=64,
                                          kernel_size=2, stride=2, padding=0, bias=True)

        self.dec1_2 = CBR2d(in_ch=2 * 64, out_ch=64)
        self.dec1_1 = CBR2d(in_ch=64, out_ch=64)

        self.fc = nn.Conv2d(in_channels=64, out_channels=23, kernel_size=1, stride=1, padding=0, bias=True)  # RGB 3이 아닌 class 개수인 23으로 해야 IoU계산시 에러 발생이 안남
